package rip.protocol.bridge.shared.updater;
...
public class UpdaterManager {
    ...
    public String getPluginUpdateDir() {
        String dir;
        if(BridgeShared.getPluginUpdateDir() == null || BridgeShared.getPluginUpdateDir().isEmpty()) {
            dir = "%user_home%/updater";
        }else {
            dir = BridgeShared.getPluginUpdateDir();
        }
    ...
    public List<File> getFilesForGroup(String group) {
        ...
        folder = new File(getPluginUpdateDir() + "/" + group);
        if(!folder.exists() || !folder.isDirectory()) return null;
    ...
    public String getRunningDirectory() {
        try {
            return BridgeShared.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath().replace("/Bridge.jar", "");
