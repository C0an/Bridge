        ...
        for (Class<?> listener : listeners) {
            if (Listener.class.isAssignableFrom(listener)) {
                try {
                    ...
                    BridgeShared.sendLog("Successfully registered listener " + listener.getSimpleName() + "!");
                } catch (Exception ex) {
                    BridgeShared.sendLog("Failed to registered listener "   + listener.getSimpleName() + ". (" + ex.getClass().getSimpleName() + ": " + ex.getMessage() + ")");
                }
            } else {
                BridgeShared.sendLog("The class: " + listener.getSimpleName() + " does not implement Listener!");
            }
        }
    }
